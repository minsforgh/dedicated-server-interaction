...
void AMultiplayCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		EnhancedInputComponent->BindAction(SpawnCubeAction, ETriggerEvent::Started, this, &AMultiplayCharacter::SpawnCube);
	}
	else
	{
...
void AMultiplayCharacter::SpawnCube(const FInputActionValue& Value)
{
	// Call the Server RPC to spawn the cube
	if (HasAuthority())
	{
		// If it's the server, directly spawn the cube
		ServerSpawnCube();
	}
	else
	{
		// If it's the client, send the request to the server
		ServerSpawnCube();
	}
}

void AMultiplayCharacter::ServerSpawnCube_Implementation()
{
	// Check if CubeClass is valid
	if (CubeClass)
	{
		...
		// Spawn the cube on the server
		AActor* SpawnedCube = GetWorld()->SpawnActor<AActor>(CubeClass, SpawnLocation, SpawnRotation);

		// Ensure the cube is replicated to all clients
		if (SpawnedCube)
		{
			SpawnedCube->SetReplicates(true);
			SpawnedCube->SetReplicateMovement(true);
		}
	}
}

// Validate the Server RPC (needed for reliable RPC calls)
bool AMultiplayCharacter::ServerSpawnCube_Validate()
{
	// Here you can add validation logic (e.g., check if the spawn location is valid)
	return true;
}
